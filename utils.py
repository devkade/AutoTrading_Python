...
def get_ohlcv(exchange='upbit', ticker = 'KRW-BTC', n=1, time=None): #코인의 정보를 불러옴, 500*n개의 정보
    dfs = [ ]
    if exchange=='upbit':
...
def Ahpr(ror: list, period: float):
    return (1+hpr(ror))**(1/period)-1

def get_period(index: pd.DatetimeIndex):
    return (index[-1]-index[0]).days

# df = get_ohlcv('binance', tickers['binance'], 5)
